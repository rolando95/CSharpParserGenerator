...
namespace CSharpParserGenerator.Controllers
{
    ...
    public class DynamicQueryController : ControllerBase
    {
        ...
        public IActionResult Query([FromQuery(Name = "filter")] string expression)
        {
            ...
            var expressionResult = GetExpression<Person>(expressionTree.Value);

            var list = Persons.Where(expressionResult);
            return Ok(list);
        }
        ...
        private Parser<ELang> CompileParser()
        {
            ...
            {
                [ELang.Ignore] = "[ \\n]+",
                [ELang.LPar] = "\\(", // Left parenthesis
                [ELang.RPar] = "\\)", // Rigt parenthesis
            ...
            var rules = new SyntaxDefinition<ELang>(new Dictionary<ELang, DefinitionRules>()
            {
                ...
                [ELang.Relational] = new DefinitionRules
                {
                    ...
                    new List<Token> { ELang.LPar, ELang.LogicalOr, ELang.RPar, new Op(o => { o[0] = o[2]; }) },
                },
        ...
        private Expression<Func<T, bool>> GetExpression<T>(ExpressionNode expression)
        {
            var parameter = Expression.Parameter(typeof(T), "x");
            var body = GetLogicalExpression<T>(parameter, expression);
            return Expression.Lambda<Func<T, bool>>(body, parameter);
        }

        private BinaryExpression GetLogicalExpression<T>(ParameterExpression parameter, ExpressionNode expression)
        {
            if (expression.isRelational)
            {
                return GetRelationalExpression(parameter, expression.Property, expression.Operation, expression.Value);
            }

            var left = GetLogicalExpression<T>(parameter, expression.Left);
            var right = GetLogicalExpression<T>(parameter, expression.Right);
        ...
        private BinaryExpression GetRelationalExpression(ParameterExpression parameter, string property, ELang operation, object value)
        {
            var member = Expression.Property(parameter, property); //x.property
